...
<div class="flex justify-end w-full"> 
    <a href="{{url('/song/create')}}" class="focus:outline-none text-red-800 bg-white hover:bg-red-800 hover:text-white focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2  border border-red-700">Add New Song</a>
</div>
        @if(session('success'))
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <img class="h-24 w-24 rounded-full" src="{{asset('uploads')}}/{{$song->Image}}"/>
                        ...
                        <td><button onclick="playA({{$song->id}})"  class="m-4">
                        ...
                        <td class=" px-6 py-4 text-red-800">
                                ...
                                <form class="px-4" action="{{ route('song.destroy', $song->id) }}" method="POST" style="display: inline-block">
                                    @csrf
                                    @method('DELETE')
